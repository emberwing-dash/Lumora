...
namespace Convai.Scripts.Runtime.Core
{
    public static class WavUtility
    {
        ...
        public static bool TryParseWavHeader(byte[] wavBytes, out WavHeader header, out int headerSize)
        {
            ...
            {
                Debug.LogWarning($"WAV data is too short to contain a header. Length: {(wavBytes == null ? 0 : wavBytes.Length)} bytes");
                return false;
            }
        ...
        {
            return CalculateDurationSeconds(wavBytes, 0);
        }

        public static float CalculateDurationSeconds(byte[] wavBytes, int sampleRateHz, short numChannels = 1, short bitsPerSample = 16)
        {
            ...
            bool hasWavSignature = wavBytes.Length >= 12 &&
            ...
            if (hasWavSignature && TryParseWavHeader(wavBytes, out WavHeader header, out int headerSize))
            {
                int bytesPerSample = header.BitsPerSample / 8;
                if (header.SampleRate <= 0 || header.NumChannels <= 0 || bytesPerSample <= 0)
                {
                    Debug.LogWarning("Invalid WAV format metadata. Falling back to raw PCM duration calculation.");
                }
                else
                {
                    int totalSamples = header.DataSize / (header.NumChannels * bytesPerSample);
            ...
            int fallbackBytesPerSample = bitsPerSample / 8;
            if (fallbackBytesPerSample <= 0) fallbackBytesPerSample = 2;

            int fallbackTotalSamples = wavBytes.Length / (numChannels * fallbackBytesPerSample);
            if (fallbackTotalSamples <= 0)
            {
                return 0f;
            }

            return (float)fallbackTotalSamples / sampleRateHz;
        }
    }
}